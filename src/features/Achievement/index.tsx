...
const Achievement: FC<Props> = ({
    ...
    if (mobile) {
        return (
            <div
                className={`max-w-200 border-2 border-main-orange h-44 max-h-44 rounded-4xl flex overflow-hidden w-full ${className}`}
                {...props}>
                <Image
                    width={1416}
                    height={945}
    ...
    if (compact) {
        return (
            <div
                ...
                onClick={onClick}>
                <div
                    className='text-center flex items-center transition-all justify-center  text-5xl uppercase bg-gradient-to-b from-[rgba(0,0,0,0.5)] hover:from-main-orange-50 hover:to-main-orange-50 to-transparent from-52%  h-full w-full '>
    ...
    return (
        <div
            className={` w-full h-[42rem]  border-5 border-main-orange rounded-6xl overflow-hidden bg-no-repeat bg-right bg-contain   ${className}`}
            style={{
                backgroundImage: `url(${curPhoto})`,
            ...
            onClick={onClick}>
            <div
                className='px-32 flex flex-col justify-around  bg-gradient-to-r from-black to-transparent from-52% h-full'>
                <div className='flex flex-col gap-8 max-w-1/2'>
                    <div className='text-7xl max-w-full truncate'>{title}</div>
                    <div
                        className='text-4xl max-h-80 overflow-y-scroll [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]'>
                        {description}
                    </div>
                </div>
