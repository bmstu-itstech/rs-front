...
const Achievement = forwardRef<HTMLDivElement, Props>(
  (
    {
      title,
      description,
    ...
    if (mobile) {
      return (
        <div
          className={`border-2 border-main-orange shrink-0 h-60 rounded-4xl flex overflow-hidden w-full ${className}`}
          {...props}
          ref={ref}
          id={String(props.id)}>
          <Image
          ...
          />
          <Divider isVertical />
          <div className='flex flex-col items-center justify-evenly  gap-1 py-2 w-full'>
            <p className='text-5xl text-center uppercase text-ellipsis line-clamp-2 max-w-11/12 w-full'>
              {title}
            </p>
            <Divider />
      ...
      <div
        ...
        <div className='px-32 py-16 flex flex-col justify-around w-full bg-gradient-to-r from-black to-transparent from-52% h-full'>
          ...
          <div className='flex h-fit gap-6 2xl:gap-14'>
            <button
              className='w-40 lg:w-60 2xl:w-72 py-5 lg:py-6 xl:py-7 lg:rounded-[3rem] 2xl:rounded-6xl bg-main-orange hover:bg-white hover:text-main-orange flex justify-center items-center cursor-pointer text-4xl duration-300'
              onClick={() => router.push(album_url)}>
              ФОТО
            </button>
            <button
              className='w-40 lg:w-60 2xl:w-72 py-5 lg:py-6 xl:py-7 lg:rounded-[3rem] 2xl:rounded-6xl bg-main-orange hover:bg-white hover:text-main-orange text-white text-4xl flex justify-center items-center cursor-pointer duration-300'
